...
- `docker-compose.selenium-grid.yml`: local Selenium Grid with Hub, Chrome node, noVNC, and video sidecar.
- `docker/selenium-node-chrome/Dockerfile`: custom Chrome node based on the official Selenium image with `xdotool` installed for browser-level shortcut activation inside the container.
...
- `docker compose -f docker-compose.selenium-grid.yml up -d --build`: start the local Docker Grid with VNC and video recording.
- `mvn test -Pgrid`: run all tests through the Docker Grid profile.
- `mvn test -Pgrid -Dtest=DuckDuckGoSearchTest`: run the demo UI test through Grid; videos land under `target/selenium-videos/`.
- `docker compose -f docker-compose.selenium-grid.yml down`: stop the local Docker Grid.
...
For Docker Grid, the browser runs inside the Chrome node container. The `grid` Maven profile uses `RemoteWebDriver`, mounts the unpacked extension at `/opt/at/extensions/aut-logs-sidepanel`, exposes the host log server as `http://host.docker.internal:17654`, and opens the Side Panel by running `docker exec logpanel-selenium-chrome ... xdotool ...` from `log.panel.open.command`. Keep these pieces aligned when changing container names, ports, extension paths, or compose service names.

When proving Grid behavior, prefer the recorded mp4 from `target/selenium-videos/` or a frame extracted from it. The expected proof shows the mobile DuckDuckGo viewport and the `AT Log Panel` Chrome Side Panel in the same browser window.
...
Do not commit generated video files or screenshots from `target/selenium-videos/`. The compose file currently uses locally available Selenium image tags (`4.31.0` plus `ffmpeg-7.1-20250414`) to avoid relying on Docker Hub during local work; update the tags intentionally and re-test VNC/video if you change them.
